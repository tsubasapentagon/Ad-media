...
export async function fetchDashboardSnapshot(params:{startDate:string;endDate:string;media?:string;category?:string;subcategory?:string;placement?:string;search?:string;limit?:number;offset?:number}):Promise<DashboardSnapshot>{
  ...
  // Supabase の無料枠では同時に重い集計を走らせると、双方が statement_timeout に
  // 到達する。広告一覧を先に完了させてから週次集計を取得し、DB負荷を競合させない。
  const performance=await fetch(`${endpoint}read_dashboard_performance`,{method:"POST",headers,body:JSON.stringify({...filters,p_search:params.search||null,p_limit:params.limit??100,p_offset:params.offset??0}),cache:"no-store"});
  if(!performance.ok) throw new Error(`広告一覧の取得に失敗しました (${performance.status})`);
  const trends=await fetch(`${endpoint}read_dashboard_trends`,{method:"POST",headers,body:JSON.stringify(filters),cache:"no-store"});
  if(!trends.ok) throw new Error(`週次分析の取得に失敗しました (${trends.status})`);
