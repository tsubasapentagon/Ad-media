...
export async function GET(request:Request){
  ...
  const placements=url.searchParams.getAll("placement");
  const placement=scope||placements.length?`__filter__:${JSON.stringify({scope:scope??null,placements})}`:undefined;
