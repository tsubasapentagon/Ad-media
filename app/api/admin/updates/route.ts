...
const targets=new Set(["all","ad_master"]);

export async function POST(request:Request){
  const denied=requireAdmin(request);if(denied)return denied;
  try{
    const body=await request.json() as {target?:string;startDate?:string;endDate?:string};
    if(!body.target||!targets.has(body.target))return Response.json({error:"更新対象が不正です"},{status:400});
    if(body.target!=="ad_master"){
    ...
    if(!token)return Response.json({error:"更新機能の接続設定がありません"},{status:503});
    const response=await fetch("https://api.github.com/repos/tsubasapentagon/Ad-media/actions/workflows/daily-sync.yml/dispatches",{
      ...
      body:JSON.stringify({ref:"main",inputs:{sync_target:body.target,start_date:body.startDate??"",end_date:body.endDate??""}}),
    });
